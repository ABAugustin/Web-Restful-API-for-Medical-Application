...
def afisareDocsFromPat(id):
    ...
    if connected.is_connected():
        cursor = connected.cursor(dictionary=True)
        query = "SELECT date_doctori.* FROM date_doctori JOIN date_programari ON date_doctori.id_doctor = date_programari.id_doctor WHERE date_programari.id_pacient =" + id
        cursor.execute(query)
        rows = cursor.fetchall()
        dtos = []
        for row in rows:
